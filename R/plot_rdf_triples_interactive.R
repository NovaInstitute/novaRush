...
plot_rdf_triples_interactive <- function(triples_df, context_map) {
  ...
  node_labels_df <- triples_df %>%
    ...
    dplyr::select(id = subject, label = object) %>%
    dplyr::distinct()

  # Extract node types (rdf:type)
  node_types_df <- triples_df %>%
    dplyr::filter(predicate == "rdf:type") %>%
    dplyr::select(id = subject, type = object) %>%
    dplyr::distinct() %>%
    # Apply prefixes to types for easier use in coloring/shaping
    dplyr::mutate(type = purrr::map_chr(type, ~apply_prefixes_for_display(.x, context_map)))
  ...
  graph_edges_df <- triples_df %>%
    dplyr::filter(!predicate %in% c("rdfs:label", "rdf:type", "survey:hasValue", "survey:isRequired", "survey:hasFieldName", "survey:hasQuestionType", "dcterms:identifier")) %>%
    dplyr::mutate(
      from = subject,
      to = object,
  ...
  visNetwork_plot <- visNetwork::visNetwork(nodes, edges, main = "RDF Graph from SurveyCTO Data") %>%
    ...
    visNetwork::visPhysics(solver = "barnesHut", stabilization = FALSE) %>% # Choose a physics solver for layout
    visNetwork::visEdges(
      smooth = TRUE,
      color = list(highlight = "#333333") # Edge highlight color now set here
    ) %>%
    visNetwork::visLayout(randomSeed = 123) %>% # For reproducible layout
    visNetwork::visInteraction(navigationButtons = TRUE, zoomView = TRUE) # Add navigation buttons and enable zoom
